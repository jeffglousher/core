...
def _is_credential_rejection(
    ...
    """Return whether api.x.ai rejected credentials with a non-401 status.

    Live probes show invalid Bearer tokens often return HTTP 400 with
    ``invalid-argument`` / ``Incorrect API key provided`` instead of 401.
    """
    if status != 400:
        return False
    ...
    message = (_provider_error_message(body) or "").lower()
    return (
        "api key" in message or "no credentials" in message or "invalid_token" in code
    )
