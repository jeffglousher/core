...
The speech entities use shared account and token handling. Tests cover speech configuration, transcription and synthesis requests, supported audio parameters, lifecycle behavior, and provider errors. This layer uses spacexai-subscription-client 0.4.0. Public speech timeout/retry tests verify stored credentials survive and the next request uses the refreshed token.
